...
def scrape_top_250(url):
    ...
    for e in dom.by_tag("td.titleColumn")[:250]: #amount of titles
        for a in e.by_tag("a"): #search for title
...
def scrape_movie_page(dom):
    ...
    print ''
    for e in dom.by_class("itemprop")[:1]: #search title
        title = plaintext(e.content)

    for e in dom.by_class("infobar"): #search duration
        for time in e.by_tag("time"):
            duration = plaintext(time.content)
            duration = duration.replace(" min","")
            
    genres = ""       
    for e in dom.by_class("infobar"): #search genres
        for x in e.by_class("itemprop"):
            genres = genres + plaintext(x.content) + ","
            
    directors = ""
    for e in dom.by_class("txt-block")[:1]:#search directors
        for x in e.by_class("itemprop"):
            directors = directors + plaintext(x.content) + ","

    writers = ""
    for e in dom.by_class("txt-block")[1:2]:#search writers
        for x in e.by_class("itemprop"):
            writers = writers + plaintext(x.content) + ","
        
    
    actors = ""
    for e in dom.by_class("txt-block")[2:3]:#search actors
        for x in e.by_class("itemprop"):
            actors = actors + plaintext(x.content) + ","
        
    for e in dom.by_class("titlePageSprite star-box-giga-star" ):
        rating =  plaintext(e.content)

    for e in dom.by_class("star-box-details"):
        n_ratings = plaintext(e[4].content).replace(" users","")
        

    # Return everything of interest for this movie (all strings as specified
    # in the docstring of this function).
    title = title.encode('ascii', 'ignore')
    directors = directors.encode('ascii', 'ignore')
    writers = writers.encode('ascii', 'ignore')
